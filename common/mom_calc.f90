module mom_calc

  implicit none

  private

  public :: mom_calc__init, mom_calc__accl, mom_calc__nvt

  logical, save :: is_init = .false.
  integer, save :: ndim, np, nsp, nxgs, nxge, nygs, nyge, nzgs, nzge, nys, nye, nzs, nze
  real(8), save :: delx, delt, c, d_delx
  real(8), allocatable :: q(:), r(:)


contains
  subroutine mom_calc__init(ndim_in,np_in,nsp_in,nxgs_in,nxge_in,nygs_in,nyge_in,nzgs_in,nzge_in,nys_in,nye_in,nzs_in,nze_in, &
                            delx_in,delt_in,c_in,q_in,r_in)
  integer, intent(in) :: ndim_in, np_in, nsp_in
  integer, intent(in) :: nxgs_in, nxge_in, nygs_in, nyge_in, nzgs_in, nzge_in, nys_in, nye_in, nzs_in, nze_in
  real(8), intent(in) :: delx_in, delt_in, c_in, q_in(nsp_in), r_in(nsp_in)

    ndim  = ndim_in
    np    = np_in
    nsp   = nsp_in
    nxgs  = nxgs_in
    nxge  = nxge_in
    nygs  = nygs_in
    nyge  = nyge_in
    nzgs  = nzgs_in
    nzge  = nzge_in
    nys   = nys_in
    nye   = nye_in
    nzs   = nzs_in
    nze   = nze_in
    delx  = delx_in
    delt  = delt_in*5d-1
    c     = c_in
    allocate(q(nsp))
    allocate(r(nsp))
    q     = q_in
    r     = r_in

    d_delx = 1d0/delx

    is_init = .true.

  end subroutine mom_calc__init

  subroutine mom_calc__accl(gp,up,uf,cumcnt,nxs,nxe)

    integer, intent(in)  :: nxs, nxe
    integer, intent(in)  :: cumcnt(nxgs:nxge+1,nys:nye,nzs:nze,nsp)
    real(8), intent(in)  :: up(ndim,np,nys:nye,nzs:nze,nsp)
    real(8), intent(in)  :: uf(6,nxgs-2:nxge+2,nys-2:nye+2,nzs-2:nze+2)
    real(8), intent(out) :: gp(ndim,np,nys:nye,nzs:nze,nsp)

    integer            :: i, j, k, ii, isp
    real(8)            :: tmpf(6,nxs-1:nxe+1,nys-1:nye+1,nzs-1:nze+1)
    real(8)            :: dh, fac1, fac1r, fac2, fac2r, gam, igam, txxx
    real(8)            :: bpx, bpy, bpz, epx, epy, epz
    real(8)            :: uvm1, uvm2, uvm3, uvm4, uvm5, uvm6
    real(8)            :: shxm, shx, shxp, shym, shy, shyp, shzm, shz, shzp

    if(.not.is_init)then
      write(6,*)'Initialize first by calling mom_calc__init()'
      stop
    endif

    !FIELDS AT (I+1/2, J+1/2, K+1/2)
!$OMP PARALLEL DO PRIVATE(i,j,k)
    do k=nzs-1,nze+1
      do j=nys-1,nye+1
        do i=nxs-1,nxe+1
           tmpf(1,i,j,k) = 2.5d-1*(+uf(1,i,j,k  )+uf(1,i,j+1,k  ) &
                                   +uf(1,i,j,k+1)+uf(1,i,j+1,k+1))
           tmpf(2,i,j,k) = 2.5d-1*(+uf(2,i,j,k  )+uf(2,i+1,j,k  ) &
                                   +uf(2,i,j,k+1)+uf(2,i+1,j,k+1))
           tmpf(3,i,j,k) = 2.5d-1*(+uf(3,i,j  ,k)+uf(3,i+1,j  ,k) &
                                   +uf(3,i,j+1,k)+uf(3,i+1,j+1,k))
           tmpf(4,i,j,k) = 5d-1*(+uf(4,i,j,k)+uf(4,i+1,j  ,k  ))
           tmpf(5,i,j,k) = 5d-1*(+uf(5,i,j,k)+uf(5,i  ,j+1,k  ))
           tmpf(6,i,j,k) = 5d-1*(+uf(6,i,j,k)+uf(6,i  ,j  ,k+1))
        enddo
      enddo
    enddo
!$OMP END PARALLEL DO

!$OMP PARALLEL DO PRIVATE(ii,i,j,k,isp,                              &
!$OMP                     dh,gam,igam,fac1,fac2,txxx,fac1r,fac2r,    &
!$OMP                     shxm,shx,shxp,shym,shy,shyp,shzm,shz,shzp, &
!$OMP                     bpx,bpy,bpz,epx,epy,epz,                   &
!$OMP                     uvm1,uvm2,uvm3,uvm4,uvm5,uvm6)
    do k=nzs,nze
    do j=nys,nye
    do i=nxs,nxe

      do isp=1,nsp

        fac1 = q(isp)/r(isp)*5d-1*delt
        txxx = fac1*fac1
        fac2 = q(isp)*delt/r(isp)

        do ii=cumcnt(i,j,k,isp)+1,cumcnt(i+1,j,k,isp)

          !second order shape function
          dh = up(1,ii,j,k,isp)*d_delx-5d-1-i
          shxm = 5d-1*(5d-1-dh)*(5d-1-dh)
          shx  = 7.5d-1-dh*dh
          shxp = 5d-1*(5d-1+dh)*(5d-1+dh)

          dh = up(2,ii,j,k,isp)*d_delx-5d-1-j
          shym = 5d-1*(5d-1-dh)*(5d-1-dh)
          shy  = 7.5d-1-dh*dh
          shyp = 5d-1*(5d-1+dh)*(5d-1+dh)

          dh = up(3,ii,j,k,isp)*d_delx-5d-1-k
          shzm = 5d-1*(5d-1-dh)*(5d-1-dh)
          shz  = 7.5d-1-dh*dh
          shzp = 5d-1*(5d-1+dh)*(5d-1+dh)

          bpx = (+(+tmpf(1,i-1,j-1,k-1)*shxm+tmpf(1,i,j-1,k-1)*shx+tmpf(1,i+1,j-1,k-1)*shxp)*shym       &
                 +(+tmpf(1,i-1,j  ,k-1)*shxm+tmpf(1,i,j  ,k-1)*shx+tmpf(1,i+1,j  ,k-1)*shxp)*shy        &
                 +(+tmpf(1,i-1,j+1,k-1)*shxm+tmpf(1,i,j+1,k-1)*shx+tmpf(1,i+1,j+1,k-1)*shxp)*shyp)*shzm &
               +(+(+tmpf(1,i-1,j-1,k  )*shxm+tmpf(1,i,j-1,k  )*shx+tmpf(1,i+1,j-1,k  )*shxp)*shym       &
                 +(+tmpf(1,i-1,j  ,k  )*shxm+tmpf(1,i,j  ,k  )*shx+tmpf(1,i+1,j  ,k  )*shxp)*shy        &
                 +(+tmpf(1,i-1,j+1,k  )*shxm+tmpf(1,i,j+1,k  )*shx+tmpf(1,i+1,j+1,k  )*shxp)*shyp)*shz  &
               +(+(+tmpf(1,i-1,j-1,k+1)*shxm+tmpf(1,i,j-1,k+1)*shx+tmpf(1,i+1,j-1,k+1)*shxp)*shym       &
                 +(+tmpf(1,i-1,j  ,k+1)*shxm+tmpf(1,i,j  ,k+1)*shx+tmpf(1,i+1,j  ,k+1)*shxp)*shy        &
                 +(+tmpf(1,i-1,j+1,k+1)*shxm+tmpf(1,i,j+1,k+1)*shx+tmpf(1,i+1,j+1,k+1)*shxp)*shyp)*shzp

          bpy = (+(+tmpf(2,i-1,j-1,k-1)*shxm+tmpf(2,i,j-1,k-1)*shx+tmpf(2,i+1,j-1,k-1)*shxp)*shym       &
                 +(+tmpf(2,i-1,j  ,k-1)*shxm+tmpf(2,i,j  ,k-1)*shx+tmpf(2,i+1,j  ,k-1)*shxp)*shy        &
                 +(+tmpf(2,i-1,j+1,k-1)*shxm+tmpf(2,i,j+1,k-1)*shx+tmpf(2,i+1,j+1,k-1)*shxp)*shyp)*shzm &
               +(+(+tmpf(2,i-1,j-1,k  )*shxm+tmpf(2,i,j-1,k  )*shx+tmpf(2,i+1,j-1,k  )*shxp)*shym       &
                 +(+tmpf(2,i-1,j  ,k  )*shxm+tmpf(2,i,j  ,k  )*shx+tmpf(2,i+1,j  ,k  )*shxp)*shy        &
                 +(+tmpf(2,i-1,j+1,k  )*shxm+tmpf(2,i,j+1,k  )*shx+tmpf(2,i+1,j+1,k  )*shxp)*shyp)*shz  &
               +(+(+tmpf(2,i-1,j-1,k+1)*shxm+tmpf(2,i,j-1,k+1)*shx+tmpf(2,i+1,j-1,k+1)*shxp)*shym       &
                 +(+tmpf(2,i-1,j  ,k+1)*shxm+tmpf(2,i,j  ,k+1)*shx+tmpf(2,i+1,j  ,k+1)*shxp)*shy        &
                 +(+tmpf(2,i-1,j+1,k+1)*shxm+tmpf(2,i,j+1,k+1)*shx+tmpf(2,i+1,j+1,k+1)*shxp)*shyp)*shzp

          bpz = (+(+tmpf(3,i-1,j-1,k-1)*shxm+tmpf(3,i,j-1,k-1)*shx+tmpf(3,i+1,j-1,k-1)*shxp)*shym       &
                 +(+tmpf(3,i-1,j  ,k-1)*shxm+tmpf(3,i,j  ,k-1)*shx+tmpf(3,i+1,j  ,k-1)*shxp)*shy        &
                 +(+tmpf(3,i-1,j+1,k-1)*shxm+tmpf(3,i,j+1,k-1)*shx+tmpf(3,i+1,j+1,k-1)*shxp)*shyp)*shzm &
               +(+(+tmpf(3,i-1,j-1,k  )*shxm+tmpf(3,i,j-1,k  )*shx+tmpf(3,i+1,j-1,k  )*shxp)*shym       &
                 +(+tmpf(3,i-1,j  ,k  )*shxm+tmpf(3,i,j  ,k  )*shx+tmpf(3,i+1,j  ,k  )*shxp)*shy        &
                 +(+tmpf(3,i-1,j+1,k  )*shxm+tmpf(3,i,j+1,k  )*shx+tmpf(3,i+1,j+1,k  )*shxp)*shyp)*shz  &
               +(+(+tmpf(3,i-1,j-1,k+1)*shxm+tmpf(3,i,j-1,k+1)*shx+tmpf(3,i+1,j-1,k+1)*shxp)*shym       &
                 +(+tmpf(3,i-1,j  ,k+1)*shxm+tmpf(3,i,j  ,k+1)*shx+tmpf(3,i+1,j  ,k+1)*shxp)*shy        &
                 +(+tmpf(3,i-1,j+1,k+1)*shxm+tmpf(3,i,j+1,k+1)*shx+tmpf(3,i+1,j+1,k+1)*shxp)*shyp)*shzp

          epx = (+(+tmpf(4,i-1,j-1,k-1)*shxm+tmpf(4,i,j-1,k-1)*shx+tmpf(4,i+1,j-1,k-1)*shxp)*shym       &
                 +(+tmpf(4,i-1,j  ,k-1)*shxm+tmpf(4,i,j  ,k-1)*shx+tmpf(4,i+1,j  ,k-1)*shxp)*shy        &
                 +(+tmpf(4,i-1,j+1,k-1)*shxm+tmpf(4,i,j+1,k-1)*shx+tmpf(4,i+1,j+1,k-1)*shxp)*shyp)*shzm &
               +(+(+tmpf(4,i-1,j-1,k  )*shxm+tmpf(4,i,j-1,k  )*shx+tmpf(4,i+1,j-1,k  )*shxp)*shym       &
                 +(+tmpf(4,i-1,j  ,k  )*shxm+tmpf(4,i,j  ,k  )*shx+tmpf(4,i+1,j  ,k  )*shxp)*shy        &
                 +(+tmpf(4,i-1,j+1,k  )*shxm+tmpf(4,i,j+1,k  )*shx+tmpf(4,i+1,j+1,k  )*shxp)*shyp)*shz  &
               +(+(+tmpf(4,i-1,j-1,k+1)*shxm+tmpf(4,i,j-1,k+1)*shx+tmpf(4,i+1,j-1,k+1)*shxp)*shym       &
                 +(+tmpf(4,i-1,j  ,k+1)*shxm+tmpf(4,i,j  ,k+1)*shx+tmpf(4,i+1,j  ,k+1)*shxp)*shy        &
                 +(+tmpf(4,i-1,j+1,k+1)*shxm+tmpf(4,i,j+1,k+1)*shx+tmpf(4,i+1,j+1,k+1)*shxp)*shyp)*shzp

          epy = (+(+tmpf(5,i-1,j-1,k-1)*shxm+tmpf(5,i,j-1,k-1)*shx+tmpf(5,i+1,j-1,k-1)*shxp)*shym       &
                 +(+tmpf(5,i-1,j  ,k-1)*shxm+tmpf(5,i,j  ,k-1)*shx+tmpf(5,i+1,j  ,k-1)*shxp)*shy        &
                 +(+tmpf(5,i-1,j+1,k-1)*shxm+tmpf(5,i,j+1,k-1)*shx+tmpf(5,i+1,j+1,k-1)*shxp)*shyp)*shzm &
               +(+(+tmpf(5,i-1,j-1,k  )*shxm+tmpf(5,i,j-1,k  )*shx+tmpf(5,i+1,j-1,k  )*shxp)*shym       &
                 +(+tmpf(5,i-1,j  ,k  )*shxm+tmpf(5,i,j  ,k  )*shx+tmpf(5,i+1,j  ,k  )*shxp)*shy        &
                 +(+tmpf(5,i-1,j+1,k  )*shxm+tmpf(5,i,j+1,k  )*shx+tmpf(5,i+1,j+1,k  )*shxp)*shyp)*shz  &
               +(+(+tmpf(5,i-1,j-1,k+1)*shxm+tmpf(5,i,j-1,k+1)*shx+tmpf(5,i+1,j-1,k+1)*shxp)*shym       &
                 +(+tmpf(5,i-1,j  ,k+1)*shxm+tmpf(5,i,j  ,k+1)*shx+tmpf(5,i+1,j  ,k+1)*shxp)*shy        &
                 +(+tmpf(5,i-1,j+1,k+1)*shxm+tmpf(5,i,j+1,k+1)*shx+tmpf(5,i+1,j+1,k+1)*shxp)*shyp)*shzp

          epz = (+(+tmpf(6,i-1,j-1,k-1)*shxm+tmpf(6,i,j-1,k-1)*shx+tmpf(6,i+1,j-1,k-1)*shxp)*shym       &
                 +(+tmpf(6,i-1,j  ,k-1)*shxm+tmpf(6,i,j  ,k-1)*shx+tmpf(6,i+1,j  ,k-1)*shxp)*shy        &
                 +(+tmpf(6,i-1,j+1,k-1)*shxm+tmpf(6,i,j+1,k-1)*shx+tmpf(6,i+1,j+1,k-1)*shxp)*shyp)*shzm &
               +(+(+tmpf(6,i-1,j-1,k  )*shxm+tmpf(6,i,j-1,k  )*shx+tmpf(6,i+1,j-1,k  )*shxp)*shym       &
                 +(+tmpf(6,i-1,j  ,k  )*shxm+tmpf(6,i,j  ,k  )*shx+tmpf(6,i+1,j  ,k  )*shxp)*shy        &
                 +(+tmpf(6,i-1,j+1,k  )*shxm+tmpf(6,i,j+1,k  )*shx+tmpf(6,i+1,j+1,k  )*shxp)*shyp)*shz  &
               +(+(+tmpf(6,i-1,j-1,k+1)*shxm+tmpf(6,i,j-1,k+1)*shx+tmpf(6,i+1,j-1,k+1)*shxp)*shym       &
                 +(+tmpf(6,i-1,j  ,k+1)*shxm+tmpf(6,i,j  ,k+1)*shx+tmpf(6,i+1,j  ,k+1)*shxp)*shy        &
                 +(+tmpf(6,i-1,j+1,k+1)*shxm+tmpf(6,i,j+1,k+1)*shx+tmpf(6,i+1,j+1,k+1)*shxp)*shyp)*shzp

          !accel.
          uvm1 = up(4,ii,j,k,isp)+fac1*epx
          uvm2 = up(5,ii,j,k,isp)+fac1*epy
          uvm3 = up(6,ii,j,k,isp)+fac1*epz

          !rotate
          gam = sqrt(c*c+uvm1*uvm1+uvm2*uvm2+uvm3*uvm3)
          igam = 1d0/gam
          fac1r = fac1*igam
          fac2r = fac2/(gam+txxx*(bpx*bpx+bpy*bpy+bpz*bpz)*igam)

          uvm4 = uvm1+fac1r*(+uvm2*bpz-uvm3*bpy)
          uvm5 = uvm2+fac1r*(+uvm3*bpx-uvm1*bpz)
          uvm6 = uvm3+fac1r*(+uvm1*bpy-uvm2*bpx)

          uvm1 = uvm1+fac2r*(+uvm5*bpz-uvm6*bpy)
          uvm2 = uvm2+fac2r*(+uvm6*bpx-uvm4*bpz)
          uvm3 = uvm3+fac2r*(+uvm4*bpy-uvm5*bpx)

          !accel.
          gp(1,ii,j,k,isp) = up(1,ii,j,k,isp)
          gp(2,ii,j,k,isp) = up(2,ii,j,k,isp)
          gp(3,ii,j,k,isp) = up(3,ii,j,k,isp)
          gp(4,ii,j,k,isp) = uvm1+fac1*epx
          gp(5,ii,j,k,isp) = uvm2+fac1*epy
          gp(6,ii,j,k,isp) = uvm3+fac1*epz
        enddo

      enddo

    enddo
    enddo
    enddo
!$OMP END PARALLEL DO

  end subroutine mom_calc__accl


  subroutine mom_calc__nvt(mom,up,np2)

    integer, intent(in)    :: np2(nys:nye,nzs:nze,nsp)
    real(8), intent(in)    :: up(ndim,np,nys:nye,nzs:nze,nsp)
    real(8), intent(inout) :: mom(7,nxgs-1:nxge+1,nys-1:nye+1,nzs-1:nze+1,1:nsp)

    integer :: ii, ih, j, jh, k, kh, isp
    real(8) :: dx, dxm, dy, dym, dz, dzm, gam

    if(.not.is_init)then
      write(6,*)'Initialize first by calling mom_calc__init()'
      stop
    endif

!$OMP PARALLEL WORKSHARE
    mom(1:7,nxgs-1:nxge+1,nys-1:nye+1,nzs-1:nze+1,1:nsp) = 0d0
!$OMP END PARALLEL WORKSHARE

    !caluculate number density at (i+1/2, j+1/2, k+1/2)
    do isp=1,nsp
      do k=nzs,nze
      do j=nys,nye
        do ii=1,np2(j,k,isp)
          ih = floor(up(1,ii,j,k,isp)*d_delx-5d-1)
          jh = floor(up(2,ii,j,k,isp)*d_delx-5d-1)
          kh = floor(up(3,ii,j,k,isp)*d_delx-5d-1)

          dx = up(1,ii,j,k,isp)-5d-1-ih
          dxm = 1d0-dx
          dy = up(2,ii,j,k,isp)-5d-1-jh
          dym = 1d0-dy
          dz = up(3,ii,j,k,isp)-5d-1-kh
          dzm = 1d0-dz

          gam = 1d0/sqrt(1d0+(+up(4,ii,j,k,isp)*up(4,ii,j,k,isp) &
                              +up(5,ii,j,k,isp)*up(5,ii,j,k,isp) &
                              +up(6,ii,j,k,isp)*up(6,ii,j,k,isp) &
                             )/(c*c))

          !N
          mom(1,ih  ,jh  ,kh,  isp) = mom(1,ih  ,jh  ,kh,  isp)+dxm*dym*dzm
          mom(1,ih+1,jh  ,kh,  isp) = mom(1,ih+1,jh  ,kh,  isp)+dx *dym*dzm
          mom(1,ih  ,jh+1,kh,  isp) = mom(1,ih  ,jh+1,kh,  isp)+dxm*dy *dzm
          mom(1,ih+1,jh+1,kh,  isp) = mom(1,ih+1,jh+1,kh,  isp)+dx *dy *dzm
          mom(1,ih  ,jh  ,kh+1,isp) = mom(1,ih  ,jh  ,kh+1,isp)+dxm*dym*dz
          mom(1,ih+1,jh  ,kh+1,isp) = mom(1,ih+1,jh  ,kh+1,isp)+dx *dym*dz
          mom(1,ih  ,jh+1,kh+1,isp) = mom(1,ih  ,jh+1,kh+1,isp)+dxm*dy *dz
          mom(1,ih+1,jh+1,kh+1,isp) = mom(1,ih+1,jh+1,kh+1,isp)+dx *dy *dz

          !Vx
          mom(2,ih  ,jh  ,kh  ,isp) = mom(2,ih  ,jh  ,kh  ,isp)+up(4,ii,j,k,isp)*gam*dxm*dym*dzm
          mom(2,ih+1,jh  ,kh  ,isp) = mom(2,ih+1,jh  ,kh  ,isp)+up(4,ii,j,k,isp)*gam*dx *dym*dzm
          mom(2,ih  ,jh+1,kh  ,isp) = mom(2,ih  ,jh+1,kh  ,isp)+up(4,ii,j,k,isp)*gam*dxm*dy *dzm
          mom(2,ih+1,jh+1,kh  ,isp) = mom(2,ih+1,jh+1,kh  ,isp)+up(4,ii,j,k,isp)*gam*dx *dy *dzm
          mom(2,ih  ,jh  ,kh+1,isp) = mom(2,ih  ,jh  ,kh+1,isp)+up(4,ii,j,k,isp)*gam*dxm*dym*dz
          mom(2,ih+1,jh  ,kh+1,isp) = mom(2,ih+1,jh  ,kh+1,isp)+up(4,ii,j,k,isp)*gam*dx *dym*dz
          mom(2,ih  ,jh+1,kh+1,isp) = mom(2,ih  ,jh+1,kh+1,isp)+up(4,ii,j,k,isp)*gam*dxm*dy *dz
          mom(2,ih+1,jh+1,kh+1,isp) = mom(2,ih+1,jh+1,kh+1,isp)+up(4,ii,j,k,isp)*gam*dx *dy *dz

          !Vy
          mom(3,ih  ,jh  ,kh  ,isp) = mom(3,ih  ,jh  ,kh  ,isp)+up(5,ii,j,k,isp)*gam*dxm*dym*dzm
          mom(3,ih+1,jh  ,kh  ,isp) = mom(3,ih+1,jh  ,kh  ,isp)+up(5,ii,j,k,isp)*gam*dx *dym*dzm
          mom(3,ih  ,jh+1,kh  ,isp) = mom(3,ih  ,jh+1,kh  ,isp)+up(5,ii,j,k,isp)*gam*dxm*dy *dzm
          mom(3,ih+1,jh+1,kh  ,isp) = mom(3,ih+1,jh+1,kh  ,isp)+up(5,ii,j,k,isp)*gam*dx *dy *dzm
          mom(3,ih  ,jh  ,kh+1,isp) = mom(3,ih  ,jh  ,kh+1,isp)+up(5,ii,j,k,isp)*gam*dxm*dym*dz
          mom(3,ih+1,jh  ,kh+1,isp) = mom(3,ih+1,jh  ,kh+1,isp)+up(5,ii,j,k,isp)*gam*dx *dym*dz
          mom(3,ih  ,jh+1,kh+1,isp) = mom(3,ih  ,jh+1,kh+1,isp)+up(5,ii,j,k,isp)*gam*dxm*dy *dz
          mom(3,ih+1,jh+1,kh+1,isp) = mom(3,ih+1,jh+1,kh+1,isp)+up(5,ii,j,k,isp)*gam*dx *dy *dz

          !Vz
          mom(4,ih  ,jh  ,kh  ,isp) = mom(4,ih  ,jh  ,kh  ,isp)+up(6,ii,j,k,isp)*gam*dxm*dym*dzm
          mom(4,ih+1,jh  ,kh  ,isp) = mom(4,ih+1,jh  ,kh  ,isp)+up(6,ii,j,k,isp)*gam*dx *dym*dzm
          mom(4,ih  ,jh+1,kh  ,isp) = mom(4,ih  ,jh+1,kh  ,isp)+up(6,ii,j,k,isp)*gam*dxm*dy *dzm
          mom(4,ih+1,jh+1,kh  ,isp) = mom(4,ih+1,jh+1,kh  ,isp)+up(6,ii,j,k,isp)*gam*dx *dy *dzm
          mom(4,ih  ,jh  ,kh+1,isp) = mom(4,ih  ,jh  ,kh+1,isp)+up(6,ii,j,k,isp)*gam*dxm*dym*dz
          mom(4,ih+1,jh  ,kh+1,isp) = mom(4,ih+1,jh  ,kh+1,isp)+up(6,ii,j,k,isp)*gam*dx *dym*dz
          mom(4,ih  ,jh+1,kh+1,isp) = mom(4,ih  ,jh+1,kh+1,isp)+up(6,ii,j,k,isp)*gam*dxm*dy *dz
          mom(4,ih+1,jh+1,kh+1,isp) = mom(4,ih+1,jh+1,kh+1,isp)+up(6,ii,j,k,isp)*gam*dx *dy *dz

          !Txx
          mom(5,ih  ,jh  ,kh  ,isp) = mom(5,ih  ,jh  ,kh  ,isp)+up(4,ii,j,k,isp)**2*gam*dxm*dym*dzm
          mom(5,ih+1,jh  ,kh  ,isp) = mom(5,ih+1,jh  ,kh  ,isp)+up(4,ii,j,k,isp)**2*gam*dx *dym*dzm
          mom(5,ih  ,jh+1,kh  ,isp) = mom(5,ih  ,jh+1,kh  ,isp)+up(4,ii,j,k,isp)**2*gam*dxm*dy *dzm
          mom(5,ih+1,jh+1,kh  ,isp) = mom(5,ih+1,jh+1,kh  ,isp)+up(4,ii,j,k,isp)**2*gam*dx *dy *dzm
          mom(5,ih  ,jh  ,kh+1,isp) = mom(5,ih  ,jh  ,kh+1,isp)+up(4,ii,j,k,isp)**2*gam*dxm*dym*dz
          mom(5,ih+1,jh  ,kh+1,isp) = mom(5,ih+1,jh  ,kh+1,isp)+up(4,ii,j,k,isp)**2*gam*dx *dym*dz
          mom(5,ih  ,jh+1,kh+1,isp) = mom(5,ih  ,jh+1,kh+1,isp)+up(4,ii,j,k,isp)**2*gam*dxm*dy *dz
          mom(5,ih+1,jh+1,kh+1,isp) = mom(5,ih+1,jh+1,kh+1,isp)+up(4,ii,j,k,isp)**2*gam*dx *dy *dz

          !Tyy
          mom(6,ih  ,jh  ,kh  ,isp) = mom(6,ih  ,jh  ,kh  ,isp)+up(5,ii,j,k,isp)**2*gam*dxm*dym*dzm
          mom(6,ih+1,jh  ,kh  ,isp) = mom(6,ih+1,jh  ,kh  ,isp)+up(5,ii,j,k,isp)**2*gam*dx *dym*dzm
          mom(6,ih  ,jh+1,kh  ,isp) = mom(6,ih  ,jh+1,kh  ,isp)+up(5,ii,j,k,isp)**2*gam*dxm*dy *dzm
          mom(6,ih+1,jh+1,kh  ,isp) = mom(6,ih+1,jh+1,kh  ,isp)+up(5,ii,j,k,isp)**2*gam*dx *dy *dzm
          mom(6,ih  ,jh  ,kh+1,isp) = mom(6,ih  ,jh  ,kh+1,isp)+up(5,ii,j,k,isp)**2*gam*dxm*dym*dz
          mom(6,ih+1,jh  ,kh+1,isp) = mom(6,ih+1,jh  ,kh+1,isp)+up(5,ii,j,k,isp)**2*gam*dx *dym*dz
          mom(6,ih  ,jh+1,kh+1,isp) = mom(6,ih  ,jh+1,kh+1,isp)+up(5,ii,j,k,isp)**2*gam*dxm*dy *dz
          mom(6,ih+1,jh+1,kh+1,isp) = mom(6,ih+1,jh+1,kh+1,isp)+up(5,ii,j,k,isp)**2*gam*dx *dy *dz

          !Tzz
          mom(7,ih  ,jh  ,kh  ,isp) = mom(7,ih  ,jh  ,kh  ,isp)+up(6,ii,j,k,isp)**2*gam*dxm*dym*dzm
          mom(7,ih+1,jh  ,kh  ,isp) = mom(7,ih+1,jh  ,kh  ,isp)+up(6,ii,j,k,isp)**2*gam*dx *dym*dzm
          mom(7,ih  ,jh+1,kh  ,isp) = mom(7,ih  ,jh+1,kh  ,isp)+up(6,ii,j,k,isp)**2*gam*dxm*dy *dzm
          mom(7,ih+1,jh+1,kh  ,isp) = mom(7,ih+1,jh+1,kh  ,isp)+up(6,ii,j,k,isp)**2*gam*dx *dy *dzm
          mom(7,ih  ,jh  ,kh+1,isp) = mom(7,ih  ,jh  ,kh+1,isp)+up(6,ii,j,k,isp)**2*gam*dxm*dym*dz
          mom(7,ih+1,jh  ,kh+1,isp) = mom(7,ih+1,jh  ,kh+1,isp)+up(6,ii,j,k,isp)**2*gam*dx *dym*dz
          mom(7,ih  ,jh+1,kh+1,isp) = mom(7,ih  ,jh+1,kh+1,isp)+up(6,ii,j,k,isp)**2*gam*dxm*dy *dz
          mom(7,ih+1,jh+1,kh+1,isp) = mom(7,ih+1,jh+1,kh+1,isp)+up(6,ii,j,k,isp)**2*gam*dx *dy *dz
        enddo
      enddo
      enddo
    enddo

  end subroutine mom_calc__nvt


end module mom_calc
